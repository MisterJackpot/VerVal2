	...
	const Euclidean = {
		...
		getAllCorrelation: async(amostraEscolhida) =>{
			...
			for(let i = 0; i < allAmostrasBut.length;i++){
				...
				mapper[ids[i]] = allDistance[i];
			}
		//	console.log(mapper)
		//<Esse code normaliza>
			let everything = await Amostras.getAllComp()	
			...
			let componentsArr = allComp.split(',');
			let allMin = Array()
			let allMax = Array()
			let allDelta = Array()
			...
			for(let j = 0; j < componentsArr.length; j++){
				...
				for(let i = 0; i < everything.length; i++){
					max = Math.max(everything[i][componentsArr[j]],max)
					min = Math.min(everything[i][componentsArr[j]],min)
				}

				allMin.push(min)
				allMax.push(max)
				allDelta.push(max-min)
			}
			for(let j = 0; j < componentsArr.length; j++){
			for(let i = 0; i < 10;i++){
				let compValue = everything[i][componentsArr[j]]
				let compNormalized = (compValue - allMin[i] )/(allDelta[i])
				normalized.push(compNormalized);
			}
		}
