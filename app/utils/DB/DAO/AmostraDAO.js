  ...
  getBytId: (id) => {
    return new Promise((resolve,error) => {
        ...
        con.connect(function(err) {
          if (err) throw err;
          console.log('Connected!');
          var sql = 'SELECT * FROM AMOSTRAS WHERE ID = '+id;
          con.query(sql,function(err, result){
            if(err) throw err;
            resolve(result);
          })
